...
| Aura lighting | Optional `positions.light` brightness and temperature, plus documented transition options | Potential dimming and warm/cool lighting controls for Aura shades only. The read-only device list reviewed did not expose Aura light objects. |
