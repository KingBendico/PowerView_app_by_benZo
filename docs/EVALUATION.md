...
The next increment fixes room-name contrast in both themes and adds Home → Saved controls: named position presets, custom groups and temporary privacy timers, with preset/group global shortcuts. Timers confirm closure before counting down, read fresh positions before restoring, and cancel on newer commands, reported changes, disconnection, sleep or quit. All visual shade and curtain controls now have visible grips at their moving edges, including fully open/closed positions; grabbing a grip preserves its starting position. Validation now has 58 core tests and 33 native checks. These move temporary privacy and saved groups/positions out of the future backlog.

The dual-rail interaction now lets users drag either edge directly with small white circular arrow grips. When their hit areas overlap, a shared grip uses the initial pull direction: up raises the top edge, down lowers the bottom edge. Reversing a drag keeps control of the chosen rail. Expandable Fine adjustment provides independent numeric fields and bottom-edge nudges/presets; Open shade / Close shade / Stop remain whole-shade actions. Taps and unrelated clicks send no command. Cancellation, keyboard focus, reported-position limits and responsive grip spacing have native coverage, including mouse input with actual pointer capture and light/dark/narrow visual review. See the current validation record for the complete suite result.

The beta.8 increment adds the read-only schedule timeline, a consolidated device health overview and session activity history. Weekday/paused filters, solar offsets and setup errors use documented gateway fields. Health uses approximate battery ranges and reported power, signal and firmware. Activity separates command acknowledgment from gateway reports and isolates histories per home. A live read-only check recognized all 16 saved schedules and 15 shades. See [Home insights](HOME_INSIGHTS.md).

The supplied gateway API was reachable and inspected read-only. Its documented operations support a schedule overview, health diagnostics and potentially a movement-speed control. They do not expose scene creation/editing, automation editing or complete scene target positions. This narrows the earlier scene/schedule roadmap; see [API capabilities](API_CAPABILITIES.md) and [saved control behavior](SAVED_CONTROLS.md). HTTP-success responses carrying command errors are now treated as failures.
