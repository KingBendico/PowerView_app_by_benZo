...
The dual-rail interaction now uses one labelled handle and a Top rail / Bottom rail selector with a highlighted edge and compact position entry. Choosing a rail sends no movement. Dragging, keyboard adjustment, nudges and presets control the selected rail; Open shade / Close shade remain explicit whole-shade actions. Impossible movements are disabled with an explanation, and the movable rail is selected at fully retracted endpoints. This resolves the ambiguity of identical handles when rails meet. Native validation now passes 35 checks, with additional light/dark/narrow visual review.
